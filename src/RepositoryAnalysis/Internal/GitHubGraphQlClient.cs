...
internal class GitHubGraphQlClient
{
        ...
        string name)
    {
        ...
        if (result.Errors.Any()) throw new($"Could not get repository {owner}/{name}", result.Errors[0].Exception);
        ...
        string name)
    {
        ...
        if (result.Errors.Any())
        {
            ...
            throw new($"Could not get repository {owner}/{name}", result.Errors[0].Exception);
        }

        if (result.Data?.Repository is null)
        {
            _logger.LogError("Could not get repository {Owner}/{Name}", owner, name);
            throw new($"Could not get repository {owner}/{name}");
        }
        ...
        string fileName)
    {
        ...
        if (result.Errors.Any())
        {
            ...
            throw new($"Could not get file {expression} for {owner}/{name}", result.Errors[0].Exception);
        }

        if (result.Data?.Repository is null)
        {
            _logger.LogError("Could not get file {File} for {Owner}/{Name}", owner, name, expression);
            throw new($"Could not get file {expression} for {owner}/{name}");
        }
        ...
        if (text is null)
        {
            ...
            throw new($"No text returned for file {expression} for {owner}/{name}");
        }
