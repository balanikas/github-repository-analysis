        ...
        RuleDiagnostics GetDiagnosis()
        {
            ...
            if (rootFile is not null)
                return nonRootFiles.Count > 1
                    ? new(Diagnosis.Info, $"found at root and {nonRootFiles.Count} files at other locations", null, rootFile.GetLink(context))
                    : new RuleDiagnostics(Diagnosis.Info, "found at root", null, rootFile.GetLink(context));

            return nonRootFiles.Count > 0
                ? new(Diagnosis.Info, $"missing at root but found {nonRootFiles.Count} files at other locations")
...
EditorConfig files are easily readable and they work nicely with version control systems.",
            AboutLink = new("about editor config", "https://editorconfig.org/")
        });
    }
}